...
_vadd32i8_(vec32i8 opa, vec32i8 opb)
{
  vec32i8 res;
...
static inline vec32i8
_vadd32i8_simd_(vec32i8 opa, vec32i8 opb)
{
  vec32i8 res;
...
_vadd64i8_(vec64i8 opa, vec64i8 opb)
{
  vec64i8 res;
  res.low  = _vadd32i8_(opa.low,  opb.low);
  res.high = _vadd32i8_(opa.high, opb.high);
  return res;
}

static inline vec64i8
_vadd64i8_simd_(vec64i8 opa, vec64i8 opb)
{
  vec64i8 res;
...
_vadd32i16_(vec32i16 opa, vec32i16 opb)
{
  vec32i16 res;
...
static inline vec32i16
_vadd32i16_simd_(vec32i16 opa, vec32i16 opb)
{
  vec32i16 res;
...
_vadd64i16_(vec64i16 opa, vec64i16 opb)
{
  vec64i16 res;
  res.low  = _vadd32i16_(opa.low,  opb.low);
  res.high = _vadd32i16_(opa.high, opb.high);
  return res; 
}

static inline vec64i16
_vadd64i16_simd_(vec64i16 opa, vec64i16 opb)
{
  vec64i16 res;
...
_vadd32i32_(vec32i32 opa, vec32i32 opb)
{
  vec32i32 res;
...
static inline vec32i32
_vadd32i32_simd_(vec32i32 opa, vec32i32 opb)
{
  vec32i32 res;
...
_vadd64i32_(vec64i32 opa, vec64i32 opb)
{
  vec64i32 res;
  res.low  = _vadd32i32_(opa.low,  opb.low);
  res.high = _vadd32i32_(opa.high, opb.high);
  return res;
}

static inline vec64i32
_vadd64i32_simd_(vec64i32 opa, vec64i32 opb)
{
  vec64i32 res;
...
verify_vaddi8()
{
  ...
  for(int i=0; i<countof(int8_table); ++i) {
    for(int j=0; j<countof(int8_table); ++ j) {
      ...
      res_32      = _vadd32i8_(opa_32, opb_32);
      res_32_simd = _vadd32i8_simd_(opa_32, opb_32);
  ...
  for(int i=0; i<countof(int8_table); ++i) {
    for(int j=0; j<countof(int8_table); ++j) { 
      ...
      res_64      = _vadd64i8_(opa_64, opb_64);
      res_64_simd = _vadd64i8_simd_(opa_64, opb_64);
...
verify_vaddi16()
{
  ...
  for(int i=0; i<countof(int16_table); ++i) {
    for(int j=0; j<countof(int16_table); ++ j) {
      ...
      res_32      = _vadd32i16_(opa_32, opb_32);
      res_32_simd = _vadd32i16_simd_(opa_32, opb_32);
  ...
  for(int i=0; i<countof(int16_table); ++i) {
    for(int j=0; j<countof(int16_table); ++ j) {
      ...
      res_64      = _vadd64i16_(opa_64, opb_64);
      res_64_simd = _vadd64i16_simd_(opa_64, opb_64);
...
verify_vaddi32()
{
  ...
  for(int i=0; i<countof(int32_table); ++i) {
    for(int j=0; j<countof(int32_table); ++ j) {
      ...
      res_32      = _vadd32i32_(opa_32, opb_32);
      res_32_simd = _vadd32i32_simd_(opa_32, opb_32);
   ...
   for(int i=0; i<countof(int32_table); ++i) {
    for(int j=0; j<countof(int32_table); ++ j) {
      ...
      res_64      = _vadd64i32_(opa_64, opb_64);
      res_64_simd = _vadd64i32_simd_(opa_64, opb_64);
