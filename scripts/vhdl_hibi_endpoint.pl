...
if(defined($chaining)) {
  ...
  system("vhdl_hibi_endpoint_db_gen.pl --entity=$entity --hibi_addr_width=$hibi_addr_width  --mem_addr_width=$mem_addr_width" .
    " --count_width=$count_width --channels=$channels --chaining;");

  if(defined($hostif)) {
    system("vhdl_hibi_endpoint_top_gen.pl --entity=$entity --lib=$lib --hostif --chaining;");
  }
  else {
    system("vhdl_hibi_endpoint_top_gen.pl --entity=$entity --lib=$lib --chaining;");
  }
}  
else {
  system("vhdl_hibi_endpoint_db_gen.pl --entity=$entity --hibi_addr_width=$hibi_addr_width  --mem_addr_width=$mem_addr_width" .
    " --count_width=$count_width --channels=$channels;");

  if(defined($hostif)) {
    system("vhdl_hibi_endpoint_top_gen.pl --entity=$entity --lib=$lib --hostif;");
  }
  else {
    system("vhdl_hibi_endpoint_top_gen.pl --entity=$entity --lib=$lib;");
  }
}
...
my $addr_width          = ceil(log(4*$channels+4)/log(2));
